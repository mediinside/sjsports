					...
					<tr>
						...
						<td>
							<label class="i-checkbox left" for="yes_agree">
								<input type="radio" name="jb_sms_flag" value="Y" id="yes_agree" selected/>
								<i class="bicon-check type"></i>
								<span class="visible">예</span>
							</label>
							<label class="i-checkbox left" for="no_agree">
								<input type="radio" name="jb_sms_flag" value="N" id="no_agree"/>
								<i class="bicon-check type"></i>
...
<script type="text/javascript">
	...
	$('#img_submit').click(function(){
		var sms_yn = $('input[name=jb_sms_flag]:checked').val();
		if (sms_yn == 'Y') {
			if ($('#jb_sms_name').val() == "") {
				alert('이름을 입력하세요');
				$('#jb_sms_name').focus();
			...
			if ($('#jb_mobile1').val() == "" || $('#jb_mobile2').val() == "" || $('#jb_mobile3').val() == "") {
				alert('연락처를 입력하세요');
				$('#jb_mobile1').focus();
				return false;
			}
