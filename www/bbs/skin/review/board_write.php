...
<form name="frm_Board" id="frm_Board" action="<?=$get_par;?>" method="post" enctype="multipart/form-data">
  ...
  <input type="hidden" id="jb_show" name="jb_show" value="B" />
  <input type="hidden" id="jb_email" name="jb_email" value="<?=$_SESSION['suseremail']?>" />
  <input type="hidden" class="i-text middle datepicker" title="작성일자 입력" placeholder="작성일자를 입력해 주세요."id="jb_etc2" name="jb_etc2" />
  ...
  <table>
        ...
        ?>    	
    		<tr>
	          ...
	          <td>
          	  <label class='noti'><input type='checkbox' value='Y' id='jb_notice_check' name='jb_notice_check' class='chk'>공지</label>
	          </td>
	        </tr>
        <? } ?>
        <tr>
