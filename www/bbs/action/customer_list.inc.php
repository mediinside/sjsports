...
if($data_list_cnt > 0) {
	for ($i=0; $i<$data_list_cnt; $i++ ) {		
			...
			$jb_sms_name     = $data_list[$i]['jb_sms_name'];
			$jb_sms_phone    = $data_list[$i]['jb_sms_phone'];
			$jb_sms_flag     = $data_list[$i]['jb_sms_flag'];
		...
		//	$secret_icon = " <img src=\"" . $GP -> IMG_PATH . "/". $skin_dir . "/image/ticon_key.gif\" border='0' align='middle'> ";	
				...
				if($jb_mb_id == "") {
					...
					if($check_id != $jb_mb_id) {
						...
						/*	$get_par = "${index_page}?jb_code=${jb_code}&jb_idx=${jb_idx}";
							...
							$get_par.= "&jb_mode=tsecret";
						*/$get_par ='javascript:alert("공개글과 자신이 작성한 비밀글만 읽을 수 있습니다.");';
						}else if (trim($check_id) == trim($rst['jb_mb_id'])) {
							$get_par = "${index_page}?jb_code=${jb_code}&jb_idx=${jb_idx}";
							$get_par.="&search_key=" . $_GET['search_key'] . "&search_keyword=${search_keyword}&page=${page}&dep1=${dep1}&dep2=${dep2}";
						} else {						
							$get_par ='javascript:alert("비공개 상담글입니다. 공개글 및 자신의 상담글만 조회가 가능합니다.");';
						}					
					} else {
						$get_par = "${index_page}?jb_code=${jb_code}&jb_idx=${jb_idx}";
						$get_par.="&search_key=" . $_GET['search_key'] . "&search_keyword=${search_keyword}&page=${page}&dep1=${dep1}&dep2=${dep2}";
			...
			if($jb_order < 100) {
				...
				$jb_no ="	<td class='num'> <span>" . $num_idx--. "</span></td>";
			}
			...
			//파일 여부
	/*		if($jb_file_code != ''){
				$jb_title = $depth_icon . $jb_title . $comment_count . $new_icon .$new_file;
			}
		*/	
			$jb_type = $GP -> ONLINE_TYPE[$jb_type];
			
			echo ("
					<tr>
						${jb_no}
						<td class='subject'><a href='" . $get_par . "'>${jb_title}</a></td>
						<td class='date'>${jb_sms_name}</a></td>
						<td class='date'>${jb_sms_phone}</a></td>
						<td class='date'>${jb_sms_flag}</a></td>
						<td class='date'>${jb_reg_date}</td>
						<td class='state'><span class='${comment_class}'>${comment_txt}</span></td>
					</tr>	
				");
